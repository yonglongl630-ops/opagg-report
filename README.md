...
日报头部展示 5 个信息卡（雪球/同花顺/金十/财联社/东方财富；Wind 板块已剔除），每张卡统一包含 **热股 TOP10** 与 **热门话题 TOP10** 两个区块（同一自选股/话题按热度排序），下方另设「今日热门板块（官方板块榜）」：
...
>
> **聚合口径**：综合热榜（热股/热门话题跨平台合并）、今日梗（高频短语）、各平台聚合 TOP10 话题
...
按使用反馈简化为：**头部 5 卡片**（雪球/同花顺/金十/财联社/东方财富，每卡热股 TOP10 + 热门话题 TOP10；财联社热股无公开个股榜故显示「暂无热股数据」，与金十一致）→ **今日热门板块（官方板块榜）** → **综合热榜面板**（五平台汇总合并：热股 TOP10 + 热门话题 TOP10 跨平台去重、带平台标签；已剔除「蒸馏热点主题 → 代表热帖」区块）→ **今日综合热词**（全文蒸馏热词 + 五大平台热词聚合）+ **今日梗/高频短语**（五平台内容，带平台标签；已剔除「蒸馏摘要」区块）→ **各平台聚合 TOP10 话题**（替代逐平台原始帖流，不单独展示每个平台）→ **up主动态与观点蒸馏汇总**（置于最底部，逐 up主 一行汇总 + 可展开的视频/评论热点/最近动态/充电分析详情）。
...
```text
https://你的用户名.github.io/opagg-report/
```
